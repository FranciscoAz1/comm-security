...
async fn send_receipt(action: Command, receipt: Receipt) -> String {
    ...
    match res {
        ...
        // Err(_) => "Error sending receipt, invalid response".to_string(),
        Err(e) => format!("Error sending receipt: {}", e),
    }
}
...
pub fn unmarshal_wave(idata: &FormData) -> Result<(String, String, String), String> {
    let gameid = idata
        .gameid
        .clone()
        .ok_or_else(|| "You must provide a Game ID".to_string())
